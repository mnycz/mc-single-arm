	subroutine trip_thru_target (narm,spectrometer,zpos, energy, theta, Eloss,
     &                               radlen, mass, typeflag,elossflag)

	implicit none
	include 'simulate.inc'
	include 'target.inc'
	integer narm, spectrometer
	integer typeflag   !1=generate eloss, 2=min, 3=max, 4=most probable
	integer elossflag  !1SIMC, 2=mos
...
c	write(6,*)'spectrometer',spectrometer
...
! ... compute distance in radiation lengths and energy loss
	...
	call New_EnergyLoss(s_target,targ%rho,targ%Z,targ%A,energy,mass,
     &                  typeflag,elossflag,Eloss_target)
	call New_EnergyLoss(s_Al,rho_Al,Z_Al,A_Al,energy,mass,typeflag,
     &       	        elossflag,Eloss_Al)
...
! ... compute distance in radiation lengths and energy loss
c	write(6,*) s_target,targ%rho,targ%Z,targ%A,energy,mass
	...
	call New_EnergyLoss(s_Al,rho_Al,Z_Al,A_Al,energy,mass,typeflag,
     &	                  elossflag,Eloss_Al)
	call New_EnergyLoss(s_air,rho_air,Z_air,A_air,energy,mass,typeflag,
     &                    elossflag,Eloss_air)
	call New_EnergyLoss(s_kevlar,rho_kevlar,Z_kevlar,A_kevlar,energy,
     &                    mass,typeflag,elossflag,Eloss_kevlar)
	call New_EnergyLoss(s_mylar,rho_mylar,Z_mylar,A_mylar,energy,mass,
     &                    typeflag,elossflag,Eloss_mylar)
...
! Incoming electron
	call trip_thru_target(1,spectrometer, z%max, ebeam, zero,targ%Eloss(1)%max
     &                        ,targ%teff(1)%max, Me, 3, 1)
	call trip_thru_target(1,spectrometer, z%min, ebeam, zero, targ%Eloss(1)%min,
     &                        targ%teff(1)%min, Me, 2, 1)
...
! ... solid target is infinitely wide
	if (.not.liquid) then
	  call trip_thru_target(2,spectrometer, z%min, energymax, the%max,
     &           targ%Eloss(2)%max, targ%teff(2)%max, Me, 3, 1)
	  call trip_thru_target(2,spectrometer, z%max, energymin, the%min,
     &           targ%Eloss(2)%min, targ%teff(2)%min, Me, 2, 1)
...
! ... side, front or side walls could be thicker (too lazy to check!)
	  ...
	  call trip_thru_target(2,spectrometer, zz, energymax, th1, E1, t1, Me, 3, 1)
	  call trip_thru_target(2,spectrometer, zz, energymax, th2, E2, t2, Me, 3, 1)
...
! ........ an intermediate z or th will do the trick).
	  ...
	  do i = 0, 3
	    call trip_thru_target (2,spectrometer, z%min+int(i/2)*(z%max-z%min), energymin,
     >			the%min+mod(i,2)*(the%max-the%min), E1, t1, Me, 2, 1)
...
! ... Minimum z_init is -radius.
	  ...
	  call trip_thru_target(2,spectrometer, zz, energymax, the%min, targ%Eloss(2)%max,
     &                          targ%teff(2)%max, Me, 3, 1)
...
! ........ code alone, for now.
	  ...
	  do i = 0, 3
	    call trip_thru_target (2,spectrometer, z%min+int(i/2)*(z%max-z%min), energymin,
     >			the%min+mod(i,2)*(the%max-the%min), E1, t1, Me, 2, 2)
	    if (E1 .lt. targ%Eloss(2)%min) then
	      targ%Eloss(2)%min = E1
	      targ%teff(2)%min = t1
	    endif
...
! ... solid target is infinitely wide
	if (.not.liquid) then
	  call trip_thru_target (3,spectrometer,z%min, energymin, thp%max, E1, t1, m, 3, 1)
	  call trip_thru_target (3,spectrometer,z%min, energymax, thp%max, E2, t2, m, 3, 1)
	  targ%Eloss(3)%max = max(E1,E2)
	  targ%teff(3)%max = max(t1,t2)

	  call trip_thru_target (3,spectrometer,z%max, energymin, thp%min, E1, t1, m, 2, 1)
	  call trip_thru_target (3,spectrometer,z%max, energymax, thp%min, E2, t2, m, 2, 1)
...
! thicker (too lazy to check!)
	  ...
	  call trip_thru_target (3,spectrometer,zz, energymin, th1, E1, t1, m, 3, 1)
	  call trip_thru_target (3,spectrometer,zz, energymin, th2, E2, t2, m, 3, 1)
	  call trip_thru_target (3,spectrometer, zz, energymax, th1, E3, t3, m, 3, 1)
	  call trip_thru_target (3,spectrometer,zz, energymax, th2, E4, t4, m, 3, 1)
...
! ........ an intermediate z or th will do the trick)
	  ...
	  do i = 0, 3
	    call trip_thru_target (3,spectrometer,z%min+int(i/2)*(z%max-z%min), energymin,
     >		thp%min+mod(i,2)*(thp%max-thp%min), E1, t1, m, 2, 1)
	  ...
	  enddo
	  call trip_thru_target (3,spectrometer,zz, energymax, th1, E1, t1, m, 2, 1)
...
! ... Minimum z_init is -radius.
	  ...
	  call trip_thru_target (3,spectrometer,zz, energymin, thp%min, E1, t1, m, 3, 1)
	  call trip_thru_target (3,spectrometer,zz, energymax, thp%min, E2, t2, m, 3, 1)
	  targ%Eloss(3)%max = max(E1,E2)
	  targ%teff(3)%max = max(t1,t2)
